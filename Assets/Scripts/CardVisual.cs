...
public class CardVisual : MonoBehaviour
{
    ...
    [Header("Card")]
    public DraggableCard parentCard;
    ...
    public void Initialize(DraggableCard target, int index = 0)
    {
        //Declarations
        parentCard = target;
    ...
    private void Select(DraggableCard card, bool state)
    {
        DOTween.Kill(2, true);
        float dir = state ? 1 : 0;
    ...
    private void BeginDrag(DraggableCard card)
    {
        if(scaleAnimations)
    ...
    private void EndDrag(DraggableCard card)
    {
        canvas.overrideSorting = false;
        transform.DOScale(1, scaleTransition).SetEase(scaleEase);
    }

    private void PointerEnter(DraggableCard card)
    {
        if(scaleAnimations)
            transform.DOScale(scaleOnHover, scaleTransition).SetEase(scaleEase);
    ...
    private void PointerExit(DraggableCard card)
    {
        if (!parentCard.wasDragged)
            transform.DOScale(1, scaleTransition).SetEase(scaleEase);
    }

    private void PointerUp(DraggableCard card, bool longPress)
    {
        if(scaleAnimations)
    ...
    private void PointerDown(DraggableCard card)
    {
        if(scaleAnimations)
            transform.DOScale(scaleOnSelect, scaleTransition).SetEase(scaleEase);
